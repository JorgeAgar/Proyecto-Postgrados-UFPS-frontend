...
export default function ValidacionCohortesMatricula() {
  ...
  const handleSeleccionarCohorte = (cohorte: CohortePagos) => {
    navigate(`/programa/pagos/matricula/cohorte/${cohorte.id}`, {
    ...
    <div className="p-6 bg-gray-100 min-h-full" style={{ fontFamily: "Segoe UI, sans-serif" }}>
      <div className="">
        ...
        <div className="mb-6 animate-fade-in">
          ...
          <h2 className="text-base font-semibold text-gray-700 mt-3">Matrícula — Cohortes</h2>
          <p className="text-sm text-neutral-400 mt-1">
            Selecciona una cohorte para gestionar la validación de pagos de matrícula.
          </p>
        </div>
        ...
        {cargando ? (
          ...
          <div className="space-y-4 animate-fade-in-up delay-100">
            {cohortes.map((cohorte, idx) => {
              const totalPorLegalizar = cohorte.totalAdmitidos;
              const pct = totalPorLegalizar > 0
                ? Math.min(100, Math.round((cohorte.totalLegalizados / totalPorLegalizar) * 100))
                : 0;
              return (
                <button
                  key={cohorte.id}
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Admitidos y Legalizados */}
                      <div className="flex gap-6 flex-wrap mb-3">
                        <div className="text-sm">
                          <span className="text-neutral-400">Admitidos: </span>
                          <span className="font-semibold text-gray-800">{cohorte.totalAdmitidos}</span>
                        </div>
                        <div className="text-sm">
                          <span className="text-neutral-400">Legalizados: </span>
                          <span className="font-semibold text-gray-800">{cohorte.totalLegalizados}</span>
                        </div>
                      </div>

                      {/* Barra de progreso legalizados */}
                      {totalPorLegalizar > 0 && (
                        <div>
                          <div className="text-xs text-neutral-400 mb-1">Legalizados / Total por legalizar</div>
                          <div className="w-full bg-neutral-200 rounded-full h-2 overflow-hidden">
                            <div className="h-2 bg-red-700 rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
                          </div>
                          <div className="text-sm mt-1.5">
                            <span className="text-neutral-400">Legalizados: </span>
                            <span className="font-semibold text-red-700">{cohorte.totalLegalizados}</span>
                            <span className="text-neutral-400"> de </span>
                            <span className="font-semibold text-gray-800">{totalPorLegalizar}</span>
                          </div>
                        </div>
                      )}
                    </div>
